...
# Urls
url <- "http://89.16.175.95/pub/cpt-city/esri/hypsometry/na/utah_1.cpt"
...
# Need to add limits: highest peak in Utah is 5000 m

nrow(make_hex)
a <- seq(0, 1000, length.out = 60) %>%
  plyr::round_any(10, f = ceiling) %>%
  unique()
b <- seq(1050, 3500, length.out = 50) %>%
  plyr::round_any(50, f = ceiling) %>%
  unique()
c <- seq(3550, 5000, length.out = 17) %>%
  plyr::round_any(50, f = ceiling) %>%
  unique()
