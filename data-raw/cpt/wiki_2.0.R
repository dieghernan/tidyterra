...
# Urls
url <- "http://89.16.175.95/pub/cpt-city/wkp/template/wiki-2.0.cpt"
...
nrow(make_hex)
bat <- c(seq(50, 3000, 500), 5000, 7500, 10000, 20000)
a <- unique(-1 * bat)

hyp <- c(seq(0, 1000, 100), seq(1000, 2500, 250), 5000, 7500, 9000)
b <- unique(hyp)
# c <- seq(4550,7000,length.out=17)
nn <- sort(unique(c(a, b)))
# nn <- unique(plyr::round_any(nn, 10, f = ceiling))

make_hex$limit <- nn

# Need to adjust bathy
hypso <- make_hex |> select(1:5)
bathy <- make_hex |> select(-c(1:5))

newbathy <- hypso
newbathy$limit <- lead(hypso$limit, default = 9999999)
names(newbathy) <- names(bathy)
