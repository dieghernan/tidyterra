...
This repository includes repo-local AI review assets under `.github/`.
When an AI task names one of these agents or skills, load the repo-local
definition first, before using any global user-level copy:

- `review-comments`: load `.github/agents/review-comments.agent.md`
  together with `.github/skills/proofread-comments/SKILL.md`.
- `review-docs`: load `.github/agents/review-docs.agent.md` together
  with `.github/skills/proofread-docs/SKILL.md`.
- `proofread-comments`: load
  `.github/skills/proofread-comments/SKILL.md`.
- `proofread-docs`: load `.github/skills/proofread-docs/SKILL.md`.

Treat `.github/agents/` and `.github/skills/` as the canonical, portable
definitions for this package. They are intended to be shared across
packages with the same AI documentation-review setup, so avoid
hard-coded local paths such as user profile directories.

If a requested repo-local agent or skill is missing, say so briefly and
fall back to the best available global or built-in workflow.
