...
test_that("Return empty geom when no results", {
  ...
  tbl2 <- tibble::tibble(geometry = "MULTIPOINT EMPTY")
...
test_that("Drop na with SpatRaster", {
  ...
  expect_false(res)
  expect_equal(nrow(tidyr::drop_na(tbl)), terra::ncell(all))
  ...
  expect_equal(nrow(tidyr::drop_na(tbl, lyr.1)), terra::ncell(lyr))
})
