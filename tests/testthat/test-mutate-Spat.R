test_that("SpatRaster mutate", {
  ...
  mod <- spatrast |> mutate(exp_lyr1 = exp(tavg_04 / 10))
...
test_that("SpatRaster mutate and check names", {
  ...
  all_eq <- all(
    names(fixed_names) == make.names(c(names(spatrast), "b2"), unique = TRUE)
  )
  ...
  expect_snapshot(fixed_names2 <- dplyr::mutate(spatrast, b2 = x.1 + x.2 + y.1))

  all_eq <- all(
    names(fixed_names2) ==
      make.names(c("x", "y", names(spatrast), "b2"), unique = TRUE)[-c(1, 2)]
  )
...
test_that("SpatVector", {
  ...
  expect_identical(mutated$cpro2, paste0(v$cpro, "-CyL"))
})
...
test_that(".keep= always retains grouping variables (#5582)", {
  ...
  df <- cbind(v[seq_len(nrow(df)), 0], df) |> group_by(z)
  expect_named(df |> mutate(a = x + 1, .keep = "none"), c("z", "a"))
  expect_equal(
