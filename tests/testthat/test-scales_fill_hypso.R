...
test_that("Continous scale tint", {
  ...
  # Use tint option
  expect_error(p + scale_fill_hypso_c(palette = "x", as_tint = TRUE))

  p2 <- p + scale_fill_hypso_c(as_tint = TRUE)
  ...
  # Reverse
  p2_rev <- p + scale_fill_hypso_c(as_tint = TRUE, direction = -1)
  mod_rev <- ggplot2::layer_data(p2_rev)$fill
  expect_true(!any(mod_rev %in% mod))

  # Alpha
  p2_alpha <- p + scale_fill_hypso_c(as_tint = TRUE, alpha = 0.5)
  mod_alpha <- ggplot2::layer_data(p2_alpha)$fill
  expect_true(all(ggplot2::alpha(mod, .5) == mod_alpha))

  # Modify limits

  p3 <- p + scale_fill_hypso_c(as_tint = TRUE, limits = c(20, 26))
...
test_that("Breaking scale tint", {
  ...
  # Use tint option
  expect_error(p + scale_fill_hypso_b(palette = "x", as_tint = TRUE))
  p2 <- p + scale_fill_hypso_b(as_tint = TRUE)
  ...
  # Reverse
  p2_rev <- p + scale_fill_hypso_b(as_tint = TRUE, direction = -1)
  mod_rev <- ggplot2::layer_data(p2_rev)$fill
  expect_true(!any(mod_rev %in% mod))

  # Alpha
  p2_alpha <- p + scale_fill_hypso_b(as_tint = TRUE, alpha = 0.5)
  mod_alpha <- ggplot2::layer_data(p2_alpha)$fill
  expect_true(all(ggplot2::alpha(mod, .5) == mod_alpha))
