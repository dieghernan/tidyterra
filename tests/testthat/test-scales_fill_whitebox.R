test_that("Discrete scale", {
  ...
  allpals_end <- lapply(allpals, function(x) {
    palplot <- p + scale_fill_whitebox_d(
      palette = x
    )
...
test_that("Continous scale", {
  ...
  # Get pals
  allpals_end <- lapply(allpals, function(x) {
    palplot <- p + scale_fill_whitebox_c(
      palette = x
    )
    mod_pal <- ggplot2::layer_data(palplot)$fill
    mod_pal
...
test_that("Breaking scale", {
  ...
  p4 <- p_init + scale_fill_whitebox_b(
    direction = -1,
    alpha = 0.7,
    breaks = br
  )
  ...
  # Get pals
  allpals_end <- lapply(allpals, function(x) {
    palplot <- p_init + scale_fill_whitebox_b(
      palette = x
    )
    mod_pal <- ggplot2::layer_data(palplot)$fill
    mod_pal
