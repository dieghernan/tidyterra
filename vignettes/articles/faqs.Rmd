...
**tidyterra** package and the potential solutions to them (mostly related to the
...
This is probably related to the tile itself rather than the package. Most base
...
tile has been reprojected, implying some sort of sampling that causes the
blurriness on your data. Also, modify the argument `maxcell` to avoid
...
<https://github.com/dieghernan/tidyterra/tree/main/data-raw> folder,
representing the information from the Corine Land Cover Dataset (2018) for the
city of Edinburgh[^3].
...
We can create other variations with binned legends and filled contours (see
...
Conceptually, the cells of a `SpatRaster` are rectangular, so it is not possible
to create a `SpatRaster` with i.e. hexagonal cells.
...
adjust the final plot, specifically, it is also needed to use `coord_sf()`:
