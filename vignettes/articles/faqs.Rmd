...
command just create your layer. The next example showcases three potential
approaches using continuous and discrete scales, as well as an alternative with
...
# Area of interest
aoi <- gadm(country = "CHE", path = tempdir(), level = 0) %>%
  project("EPSG:3857")

# Tile
rgb_tile <- get_tiles(aoi,
  crop = TRUE, provider = "Esri.WorldShadedRelief",
  zoom = 8, project = FALSE
)

# Clim (mean prec)
clim <- worldclim_country("CHE", var = "prec", path = tempdir()) %>%
  project(rgb_tile) %>%
  mask(aoi) %>%
  terra::mean()
...
p3 <- ggplot(aoi) +
  geom_spatraster_rgb(data = rgb_tile, alpha = 1) +
  geom_spatraster_contour_filled(data = clim, , bins = 5) +
  geom_spatvector(fill = NA) +
  coord_sf(expand = FALSE) +
  scale_fill_whitebox_d(palette = "deep", alpha = 0.5) +
  labs(
    subtitle = "Filled contour",
...
(p1 / p2 / p3) +
  plot_annotation(
    title = "Average precipitation in Switzerland",
    subtitle = "Shaded relief overlay",
    caption = paste0(
      c(
