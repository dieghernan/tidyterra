...
can ask for help or search existing questions using the following links.
...
Use `geom_spatraster_contour_text()` for contour labels. This function is
experimental:
...
```{r}
#| label: fig-metr
#| fig-cap: "Alternative (**metR**): contour labeling combining **tidyterra** and
...
base tiles are usually provided in **EPSG:3857**, so verify that your tile uses
this CRS rather than a different one. If your tile is not in **EPSG:3857**, it
has likely been reprojected, which involves resampling and causes blurriness. To
avoid extra resampling, increase the `maxcell` argument and ensure the
**ggplot2** map uses **EPSG:3857** with `ggplot2::coord_sf(crs = 3857)`:
