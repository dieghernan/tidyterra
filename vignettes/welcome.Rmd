...
[**tidyverse**](https://tidyverse.org/) for `SpatRaster` and `SpatVector`
objects created with the [**terra**](https://CRAN.R-project.org/package=terra)
package. It also adds specific `geom_spat*()` functions for plotting these kinds
...
working with this kind of objects, so it is **recommended** also to learn a bit
...
Especially when working with `filter()` and `mutate()` methods, it can be slow.
...
Throughout the process, the essential properties of the `SpatRaster` (number of
cells, columns and rows, extent, resolution and coordinate reference system)
have not been modified. Other methods such as `filter()`, `slice()` or
`drop_na()` can modify these properties, as they would do when applied to a data
...
As in the case of `SpatRaster`, basic properties such as the geometry and the
CRS are preserved.
...
has the CRS defined (i.e. `terra::crs(a_rast) != ""`), the geom uses
`ggplot2::coord_sf()`, and may also be reprojected for adjusting the coordinates
...
<div class="figure">
<img src="./rgb-1.png" alt="Plotting an RGB SpatRaster" width="100%" />
<p class="caption">Plotting an RGB SpatRaster</p>
</div>
...
<div class="figure">
<img src="./aggregate-1.png" alt="Dissolving SpatVectors by group" width="100%" />
...
<div class="figure">
<img src="./aggregate-2.png" alt="Dissolving SpatVectors by group (keeping internal boundaries)" width="100%" />
